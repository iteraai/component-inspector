...
type IterationInspectorRuntimeCaptureRuntime = {
  start: () => void;
  stop: () => void;
};

type CreateIterationInspectorRuntime = (options: {
  allowSelfMessaging?: boolean;
  hostOrigins?: readonly string[];
}) => IterationInspectorRuntimeCaptureRuntime;

type BuildIterationElementSelection = (
  element: Element,
) => Pick<IterationElementSelection, 'element'>;

export type IterationInspectorRuntimeCaptureSuiteOptions = {
  ITERATION_INSPECTOR_CHANNEL: string;
  buildIterationElementSelection: BuildIterationElementSelection;
  createIterationInspectorRuntime: CreateIterationInspectorRuntime;
};

export const describeIterationInspectorRuntimeCaptureSuite = ({
  ITERATION_INSPECTOR_CHANNEL,
  buildIterationElementSelection,
  createIterationInspectorRuntime,
}: IterationInspectorRuntimeCaptureSuiteOptions) => {
