...
type VueTreeMappingOptions = Readonly<{
  traversalResult: VueTraversalResult;
  nodeIdByRecordKey: ReadonlyMap<string, string>;
}>;

const toIncludedRecords = (options: VueTreeMappingOptions) => {
  const includedRecords: VueTraversalRecord[] = [];
  const includedRecordByKey = new Map<string, VueTraversalRecord>();
...
const toParentRecordKeyByRecordKey = (
  includedRecords: VueTraversalRecord[],
  includedRecordByKey: ReadonlyMap<string, VueTraversalRecord>,
) => {
...
export const mapVueTraversalToTreeSnapshot = (
  options: VueTreeMappingOptions,
